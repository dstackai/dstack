...
from dstack._internal.proxy.repos.models import Service
from dstack._internal.proxy.testing.common import make_project, make_service


def make_client(
    repo: BaseProxyRepo,  # TODO: repo type?
) -> httpx.AsyncClient:
    app = make_app(repo)
...
async def test_collect_stats(services: Iterable[Service], collector_stats, expected_response):
    ...
    with patch("dstack._internal.proxy.services.stats.StatsCollector.collect") as collect_mock:
