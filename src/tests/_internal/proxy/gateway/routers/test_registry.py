...
class TestRegisterService:
    ...
    async def test_register_with_rate_limits(self, tmp_path: Path, system_mocks: Mocks) -> None:
        ...
        assert resp.status_code == 200
        conf = (tmp_path / "443-test-run.gtw.test.conf").read_text()
        assert "limit_req_zone $binary_remote_addr zone=zone_0:10m rate=150r/m;" in conf
        assert "limit_req_zone $http_x_api_key zone=zone_1:10m rate=60r/m;" in conf
        assert "location /a {" in conf
        assert "location /b {" in conf
        assert "location / {" in conf
        assert "limit_req zone=zone_0 burst=5 nodelay;"
        assert "limit_req zone=zone_1 burst=0 nodelay;"
    ...
    async def test_register_with_root_rate_limit(
        ...
        assert resp.status_code == 200
        conf = (tmp_path / "443-test-run.gtw.test.conf").read_text()
        assert "limit_req_zone $binary_remote_addr zone=zone_0:10m rate=60r/m;" in conf
        assert "location / {" in conf
        assert "limit_req zone=zone_0 burst=1 nodelay;"
