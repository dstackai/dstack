...
class TestGetRunGpus:
    ...
    async def test_returns_gpus_without_group_by(
        ...
        offer_runpod = create_gpu_offer(BackendType.RUNPOD, "RTX4090", 24576, 0.35, spot=True, region="us-east-1")
    ...
    async def test_returns_empty_gpus_when_no_offers(
        ...
        with patch("dstack._internal.server.services.backends.get_project_backends") as m:
            ...
            response = await client.post(
                f"/api/project/{project.name}/runs/gpus",
                headers=get_auth_headers(user.token),
                json={"run_spec": run_spec.dict()},
            )
    ...
    async def test_exact_aggregation_values(
        ...
        offer_t4_spot = InstanceOfferWithAvailability(
            ...
            instance=InstanceType(
                ...
                resources=Resources(
                    ...
                    memory_mib=16384,
                    spot=True,
                    gpus=[Gpu(name="T4", memory_mib=16384, vendor=gpuhunt.AcceleratorVendor.NVIDIA)]
                ),
            ),
            region="us-west-2",
            price=0.30,
        ...
        with patch("dstack._internal.server.services.backends.get_project_backends") as m:
            ...
            backend_mock_aws.TYPE = BackendType.AWS
            backend_mock_aws.compute.return_value.get_offers_cached.return_value = [
                offer_t4_spot, offer_t4_ondemand, offer_t4_quota, offer_t4_multi
            ]
            
            backend_mock_runpod = Mock()
            backend_mock_runpod.TYPE = BackendType.RUNPOD
            backend_mock_runpod.compute.return_value.get_offers_cached.return_value = [
                offer_runpod_rtx_east, offer_runpod_rtx_eu, offer_runpod_t4_east
            ]
            ...
            for gpu in count_grouped_data["gpus"]:
                if gpu["name"] == "T4" and gpu["count"]["min"] == 1 and gpu["count"]["max"] == 1:
                    ...
                    t4_multi_group = gpu
                elif gpu["name"] == "RTX4090" and gpu["count"]["min"] == 1 and gpu["count"]["max"] == 1:
            ...
            t4_runpod = next((gpu for gpu in backend_data["gpus"] if gpu["name"] == "T4" and gpu.get("backend") == "runpod"), None)
            t4_aws = next((gpu for gpu in backend_data["gpus"] if gpu["name"] == "T4" and gpu.get("backend") == "aws"), None)
            rtx_runpod = next((gpu for gpu in backend_data["gpus"] if gpu["name"] == "RTX4090" and gpu.get("backend") == "runpod"), None)
            ...
            t4_region_group = next((gpu for gpu in region_data["gpus"] if gpu["name"] == "T4"), None)
            rtx_region_group = next((gpu for gpu in region_data["gpus"] if gpu["name"] == "RTX4090"), None)
