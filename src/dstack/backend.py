...
class AwsBackend(Backend):
    ...
    def __logs_client(self) -> BaseClient:
        session = boto3.Session(profile_name=self.backend_config.profile_name,
                                region_name=self.backend_config.region_name)
        return session.client("logs")

    def __create_log_group_if_not_exists(self, log_group_name: str):
        client = self.__logs_client()
        ...
        if not response["logGroups"]:
            client.create_log_group(
                ...
                tags={
                    'owner': 'dstack',
                    'dstack_bucket': 'self.backend_config.bucket_name',
                }
            )

    def __create_log_stream(self, log_group_name: str, run_name: str):
        client = self.__logs_client()
        client.create_log_stream(
            logGroupName=log_group_name,
            logStreamName=run_name,
        )
    ...
    def create_run(self, repo_user_name: str, repo_name: str):
        ...
        try:
            ...
            if hasattr(e, "response") and e.response.get("Error") and e.response["Error"].get("Code") == "NoSuchKey":
                ...
                raise e
        run_name = f"{name}-{count + 1}"
        log_group_name = f"/dstack/jobs/{self.backend_config.bucket_name}/{repo_user_name}/{repo_name}"
        self.__create_log_group_if_not_exists(log_group_name)
        self.__create_log_stream(log_group_name, run_name)
        return run_name
...
"""
    ...
    def __log_message(self,
        ...
        if job_id and job_id not in job_host_names:
            job = self.get_job(job_id, repo_user_name, repo_name)
    ...
    def poll_logs(self, repo_user_name: str, repo_name: str, run_name: str, start_time: int, attached: bool) -> \
        ...
        client = self.__logs_client()
        filter_logs_events_kwargs = {
            "logGroupName": f"/dstack/jobs/{self.backend_config.bucket_name}/{repo_user_name}/{repo_name}",
        ...
        try:
            if attached:
                for event in _do_filter_log_events(client, filter_logs_events_kwargs):
                    log_message = self.__log_message(event["message"], repo_user_name, repo_name, event.get("job_id"),
                ...
                for page in paginator.paginate(**filter_logs_events_kwargs):
                    for event in page["events"]:
                        log_message = self.__log_message(event["message"], repo_user_name, repo_name,
                                                         event.get("job_id"),
                                                         job_host_names, job_ports, job_apps)
                        yield LogEvent(event["timestamp"], event.get("job_id"), log_message,
                                       LogEventSource.STDOUT if event["source"] == "stdout" else LogEventSource.STDERR)
