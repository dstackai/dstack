...
async def register_service(
    ...
    async with lock:
        ...
        await nginx.register(await get_nginx_service_config(project_name, service))
        await repo.set_service(project_name, service)
...
async def unregister_replica(
    ...
    async with lock:
        ...
        results = await asyncio.gather(
            ...
            # Update Nginx config
            nginx.register(await get_nginx_service_config(project_name, service)),
            return_exceptions=True,
        )
        for exc in results:
            if isinstance(exc, Exception):
...
async def get_nginx_service_config(
    project_name: str, service: models.Service
) -> ServiceSiteConfig:
