...
def get_tag_heads(s3_client: BaseClient, bucket_name: str, repo_user_name: str, repo_name: str):
    ...
    response = s3_client.list_objects_v2(Bucket=bucket_name, Prefix=lKeyPrefix)
    tag_heads = []
...
def get_tag_head(s3_client: BaseClient, bucket_name: str, repo_user_name: str, repo_name: str, tag_name: str) -> \
        Optional[TagHead]:
    prefix = f"tags/{repo_user_name}/{repo_name}"
    lKeyPrefix = f"{prefix}/l;{tag_name};"
    response = s3_client.list_objects_v2(Bucket=bucket_name, Prefix=lKeyPrefix)
    if "Contents" in response:
        run_name, workflow_name, provider_name, created_at, artifacts = tuple(
            response["Contents"][0]["Key"][len(lKeyPrefix):].split(';'))
...
def _l_key(tag_head: TagHead) -> str:
    prefix = f"tags/{tag_head.repo_user_name}/{tag_head.repo_name}"
    lKey = f"{prefix}/l;{tag_head.tag_name};" \
           f"{tag_head.run_name};" \
...
def create_tag_from_run(s3_client: BaseClient, bucket_name: str, repo_user_name: str, repo_name: str,
    ...
    for job_head in job_heads:
        job = jobs.get_job(s3_client, bucket_name, repo_user_name, repo_name, job_head.get_id())
        tag_jobs.append(job)
...
def delete_tag(s3_client: BaseClient, bucket_name: str, repo_user_name: str, repo_name: str, tag_head: TagHead):
    tag_jobs = []
    job_heads = jobs.get_job_heads(s3_client, bucket_name, repo_user_name, repo_name, tag_head.run_name)
    for job_head in job_heads:
        job = jobs.get_job(s3_client, bucket_name, repo_user_name, repo_name, job_head.get_id())
        tag_jobs.append(job)
    lKey = _l_key(tag_head)
    s3_client.delete_object(Bucket=bucket_name, Key=lKey)
    for job in tag_jobs:
        job.tag_name = None
        jobs.update_job(s3_client, bucket_name, job)


def create_tag_from_local_dirs(s3_client: BaseClient, logs_client: BaseClient, bucket_name: str, repo: Repo,
    ...
    tag_head = TagHead(repo.repo_user_name, repo.repo_name, tag_name, run_name, job.workflow_name,
