...
func ApplyDiff(ctx context.Context, dir, patch string) error {
	...
	for _, fileInfo := range files {
		...
		if fileInfo.OldName != "" {
			...
			input = oldFile
			if err != nil {
				return fmt.Errorf("open file %s: %w", fileInfo.OldName, err)
			}
		}
		...
		if err != nil {
			...
			return fmt.Errorf("diff applier error for file %s (%s): %w", fileInfo.OldName, aes, ae)
		}
		...
		if !fileInfo.IsDelete {
			...
			if err != nil {
				return fmt.Errorf("write file %s: %w", fileInfo.NewName, err)
			}
