...
class JupyterLabProvider(Provider):
    def __init__(self):
    ...
    def _commands(self):
        commands = ["pip install jupyterlab" + (f"=={self.version}" if self.version else "")]
        if self.requirements:
            commands.append("pip install -r " + self.requirements)
        commands.append(
            f"jupyter lab --allow-root --port $JOB_PORT_0 --no-browser --ip $JOB_HOSTNAME"
        )
        return commands


if __name__ == '__main__':
    provider = JupyterLabProvider()
    provider.start()
