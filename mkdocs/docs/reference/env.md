...
- `DSTACK_AGENT_ANTHROPIC_API_KEY`{ #DSTACK_AGENT_ANTHROPIC_API_KEY } – The Anthropic API key used by the endpoint preset agent. Defaults to `None`.
- `DSTACK_AGENT_CLAUDE_USE_EXISTING_AUTH`{ #DSTACK_AGENT_CLAUDE_USE_EXISTING_AUTH } – Uses the existing `claude` login for endpoint preset creation. Defaults to `false`.
