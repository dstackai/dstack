...
async def save_credentials_repos(hub_name: str, repo_address: RepoAddressHUB, repo_credentials: RepoCredentialsHUB):
    hub = await get_hub(hub_name=hub_name)
    backend = get_backend(hub)
    backend.save_repo_credentials(
        repo_address=RepoAddress(repo_host_name=repo_address.repo_host_name,
