...
@router.get("/{hub_name}/tags/list/heads", dependencies=[Depends(Scope("tags:list:read"))],
            response_model=List[TagHead])
async def list_heads_tags(hub_name: str, repo_address: RepoAddress):
    pass


@router.get("/{hub_name}/tags/get", dependencies=[Depends(Scope("tags:get:read"))], response_model=TagHead)
async def get_tags(hub_name: str, repo_address: RepoAddress, tag_name: str):
    pass


@router.get("/{hub_name}/tags/delete", dependencies=[Depends(Scope("tags:delete:write"))], response_model=TagHead)
...
async def add_tags(hub_name: str, repo_address: RepoAddress,
                   tag_name: str,
                   run_name: str,
                   run_jobs: List[str]):
    pass
