...
@router.get("/{hub_name}/tags/list/heads",
...
@router.get("/{hub_name}/tags/{tag_name}",
...
@router.post("/{hub_name}/tags/{tag_name}/delete",
...
@router.post("/{hub_name}/tags/add/run",
             dependencies=[Depends(Scope("tags:add:write"))])
async def add_tags(hub_name: str, body: AddTagRun):
    hub = await get_hub(hub_name=hub_name)
    backend = get_backend(hub)
    backend.add_tag_from_run(repo_address=body.repo_address,
                             tag_name=body.tag_name,
                             run_name=body.run_name,
                             run_jobs=body.run_jobs)


@router.post("/{hub_name}/tags/add/path",
             dependencies=[Depends(Scope("tags:add:write"))])
async def add_tags(hub_name: str, body: AddTagPath):
    hub = await get_hub(hub_name=hub_name)
    backend = get_backend(hub)
    backend.add_tag_from_local_dirs(repo_data=body.repo_data,
                                    tag_name=body.tag_name,
                                    local_dirs=body.local_dirs
                                    )
