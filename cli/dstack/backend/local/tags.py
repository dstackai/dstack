...
def get_tag_head(path: str, repo_address: RepoAddress, tag_name: str) \
        -> Optional[TagHead]:
    root = os.path.join(path, "tags", repo_address.path())
...
def create_tag_from_run(path: str, repo_address: RepoAddress,
...
def create_tag_from_local_dirs(path: str,
                               repo_data: RepoData, tag_name: str, local_dirs: List[str]):
    ...
    for index, local_path in enumerate(local_paths):
        artifacts.upload_job_artifact_files(path, repo_data, job.job_id, tag_artifacts[index],
                                            local_path)
