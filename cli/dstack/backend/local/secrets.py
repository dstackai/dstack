...
from dstack.backend.local.common import list_objects, put_object, get_object, delete_object, get_secret_value, create_secret, put_secret_value, delete_secret
...
def get_secret(path: str, repo_address: RepoAddress, secret_name: str) \
        -> Optional[Secret]:
    root = os.path.join(path, "secrets", repo_address.path())
    try:
        return Secret(secret_name, get_secret_value(
...
def remove_secret(path: str, repo_address: RepoAddress,
                  secret_name: str):
    root = os.path.join(path, "secrets", repo_address.path())
    delete_secret(
        SecretId=f"/dstack/secrets/{repo_address.path()}/{secret_name}",
