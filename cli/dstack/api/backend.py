...
def get_all_backends(repo: Optional[RepoRef]):
    return [backend_cls(repo=repo) for backend_cls in backends_classes]


def list_backends(repo: Optional[RepoRef], all_backend: bool = False) -> List[Backend]:
    return [backend for backend in get_all_backends(repo) if all_backend or backend.loaded]


def list_remote_backends(repo: Optional[RepoRef]) -> List[RemoteBackend]:
    return [b for b in list_backends(repo) if b.type is BackendType.REMOTE]


def dict_backends(repo: Optional[RepoRef], all_backend: bool = False) -> Dict[str, Backend]:
    return {backend.name: backend for backend in list_backends(repo, all_backend=all_backend)}


def get_backend_by_name(repo: RepoRef, name: str) -> Optional[Backend]:
    return dict_backends(repo=repo).get(name)


def get_current_remote_backend(repo: Optional[RepoRef] = None) -> Optional[RemoteBackend]:
...
def get_local_backend(repo: RepoRef) -> LocalBackend:
