...
class JobHead(JobRef):
    def __init__(self, job_id: str, repo_address: RepoAddress, run_name: str, workflow_name: Optional[str],
                 provider_name: str, local_repo_user_name: Optional[str], status: JobStatus, submitted_at: int,
...
class Job(JobHead):
    def __init__(self, job_id: Optional[str], repo_data: RepoData, run_name: str, workflow_name: Optional[str],
                 provider_name: str, local_repo_user_name: Optional[str], local_repo_user_email: Optional[str],
    ...
    def serialize(self) -> dict:
        ...
        job_data = {
            ...
            } for a in self.app_specs] if self.app_specs else [],
    ...
    def unserialize(job_data: dict):
        ...
        if job_data.get("deps"):
            for dep in job_data["deps"]:
                dep_spec = DepSpec(
                    RepoAddress(dep["repo_host_name"], dep.get("repo_port") or None, dep["repo_user_name"],
        ...
        app_specs = ([
            AppSpec(a["port_index"], a["app_name"], a.get("url_path") or None, a.get("url_query_params") or None)
        ...
        job = Job(job_data["job_id"], RepoData(job_data["repo_host_name"], job_data.get("repo_port") or None,
                      ...
                      job_data.get("runner_id") or None, job_data.get("request_id") or None,
